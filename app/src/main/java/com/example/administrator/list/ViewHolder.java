...
public class ViewHolder implements Serializable {
		...
		if (a == 1) {
			...
			view.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					if (anli.isA()) {
						view.setImageResource(R.mipmap.checkbox_normal);
						jiaGe = jiaGe - anli.getPkCaseID();
						anli.setA(false);
					} else {
						view.setImageResource(R.mipmap.checkbox_pressed);
						jiaGe = jiaGe + anli.getPkCaseID();
						anli.setA(true);
					}
					vID.setText(jiaGe + "");
	...
	public void setZiXuan(int viewID, int position) {
		TextView view = getView(viewID);
		if (mPosition == position) {
			view.setBackgroundResource(R.mipmap.zixuan_l);
		} else {
			view.setBackgroundResource(R.mipmap.zixuan_s);
		}
	}
